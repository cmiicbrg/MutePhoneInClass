...
public class KUSSS {
	...
	public LVA[] getLVAs(Semester semester) throws Exception
	{
		...
		try
		{
			...
			String source = handler.getHTML("https://www.kusss.jku.at/kusss/listmystudentlvas.action"); 
			
			ArrayList<String> linkList = new ArrayList<String>();
			
			while(source.indexOf("<a href=\"sz-lvadetail-overview.action?courseId=") != -1)
			{
				source = source.substring(source.indexOf("<a href=\"sz-lvadetail-overview.action?courseId=") + 9);
				linkList.add("https://www.kusss.jku.at/kusss/" + source.substring(0, source.indexOf("\"")));
			}
	...
	public TimeTable getTimeTable(Semester sem) throws Exception
	{
		...
		try
		{
			...
			String ical = handler.getHTML("https://www.kusss.jku.at/kusss/ical-multi-sz.action?openIdical.category.mycourses=true", params);
	...
	public static void switchSemester(KUSSSHandler kusssHandler, Semester sem) throws Exception
	{
		...
		try
		{
			kusssHandler.getHTML("https://www.kusss.jku.at/kusss/select-term.action", new String[][]{{"term", sem.getShort()}});
		}
	...
	{	
		...
		try
		{
			...
			String source = handler.getHTML("https://www.kusss.jku.at/kusss/notenauskunft.action", new String[][]{{"months", "0"}});
			...
			while(source.indexOf("class=\"darkcell\">") != -1)
			{
				...
				title = source.substring(source.indexOf("<strong>") + 8, source.indexOf("</strong>")).trim();
				...
				DateTime date = new DateTime(Integer.parseInt(dateStr.substring(6)), 
						Integer.parseInt(dateStr.substring(3, 5)), Integer.parseInt(dateStr.substring(0,2)));
	...
	public Semester[] getSemesters() throws Exception
	{
		...
		{
			handler.login();
			String source = handler.getHTML("https://www.kusss.jku.at/kusss/timetable-start.action");
			handler.logout();
			
			source = source.substring(source.indexOf("<label class=\"hideme\" for=\"term\">Semester</label>"));
			int j = 0;
